...
import edu.wpi.cs.wpisuitetng.modules.logger.ModelMapper.MapCallback;

/**
 * Responsible for filling in a changeset after being passed to
 * {@link ModelMapper#map(Model, Model, MapCallback)}
 */
public class ChangesetCallback implements MapCallback {

	private final Changeset changeset;
	private boolean wasCalled = false;

	// don't add these fields as changes
	private static final Set<String> dontRecord = new HashSet<String>(
			Arrays.asList("events", "lastModifiedDate"));

	/**
	 * Create a callback that will fill in the given changeset.
	 * 
	 * @param changeset
	 *            The changeset to add changes to
	 */
	public ChangesetCallback(Changeset changeset) {
	...
	public Object call(Model source, Model destination, String fieldName,
		...
		if (!dontRecord.contains(fieldName)) {
			if (!objectsEqual(sourceValue, destinationValue)) {
				 ...
				 * remember that fields from updated defect are being copied to
				 * old one destinationValue is the old value
				 */
				changeset.getChanges().put(fieldName,
						new FieldChange<Object>(destinationValue, sourceValue));
			}
		}
		return sourceValue;
	}
