...
/**
 * Action involved with saving an edit to the values in the table
 * 
 */
	...
	public void actionPerformed(final ActionEvent e) {
		...
		for (int i = 0; i < changedRows.length; i++) {
			if (changedRows[i]) {
				...
				final String newName = (String) tableView.getTable().getModel()
						.getValueAt(i, 1);
				...
				try {
					...
					try {
						reqToChange.setType(Type.valueOf(((String) tableView
					...
					try {
						reqToChange.setPriority(Priority
								.valueOf(((String) tableView.getTable()
					...
					if (reqToChange.getStatus().equals(Status.DELETED)
							&& reqToChange.getpUID().size() > 0) {
						parentReq = RequirementDatabase.getInstance().get(
								reqToChange.getpUID().get(0));
