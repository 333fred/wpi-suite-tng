...
public class MakeTaskPanel extends JPanel {
	...
	public MakeTaskPanel(Requirement model, DetailPanel parentView) {
		...
		taskField.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent event) {
				...
				if (event.getKeyCode() == KeyEvent.VK_ENTER) {
					taskField.append("\n");
					/*
					 * if (event.isShiftDown()) { // user is doing a shift +
					 * enter taskField.append("\n"); } else { // save the note
					 * addtask.doClick(0); }
					 */
					event.consume();
				}
		...
		// addtask.setAction(new SaveNoteAction(new SaveNoteController(this,
		// model, parentView)));
		...
		layout.putConstraint(SpringLayout.NORTH, addtask, VERTICAL_PADDING,
				SpringLayout.SOUTH, taskFieldPane);
