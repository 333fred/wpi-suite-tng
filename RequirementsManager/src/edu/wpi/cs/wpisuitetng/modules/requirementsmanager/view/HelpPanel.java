...
public class HelpPanel extends Tab {
	...
	public HelpPanel() {
		...
		/*
		 * helpPane.setContentType("text/html"); //Prepare for html and set the
		 * bounds helpPane.setBounds(new Rectangle(10, 10, 260, 365));
		 */

		URL index = null; // Will be our main index page

		File input = new File("index.html"); // Html file is our input
		String path = new String("file:///" + input.getAbsolutePath()); // Grab
																		// the
																		// url
																		// of
																		// the
		...
		/*
		 * try { helpPane.setPage(index); //Try to set the page to our index
		 * (Will look awful, no CSS support) } catch (IOException e) { // TODO
		...
		if (desktop != null && desktop.isSupported(Desktop.Action.BROWSE)) { // If
																				...
																				// supported
			try {
				desktop.browse(index.toURI()); // Convert link to identifier and
